...
async def find_brand_telegram_channels(brand: str, client=None, max_per: int = 3) -> list[str]:
    ...
    # 3. Public-web verification (t.me/s) — ALWAYS attempted (not just when the
    #    other sources are empty): it loads the channel's public preview to confirm
    #    a real handle, catching brands Telegram in-app search / DDGS miss or match
    #    weakly, and works even when the Telethon session is down. Verified handles
    #    are appended (deduped) so a weak Telegram match can't shadow a real one.
    try:
